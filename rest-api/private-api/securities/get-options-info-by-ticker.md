...
{% hint style="warning" %}
In order to retrieve information about a particular option by providing their ticker symbol, you must use an [authorization token](../authentication/) of an administrator. Using authorization tokens of regular users will lead to the 401 status code.
{% endhint %}
...
4. **symbol** \(path\). This is the ticker symbol of the option whose information you'd like to retrieve. This ticker symbol can be fetched using the [API request that lists filtered options](get-sorted-options.md).
