...
{% hint style="warning" %}
In order to add a specific security \(by its ticker symbol\) to a specific watchlist of a particular user, you must use an [authorization token]() of an administrator. Using authorization tokens of regular users will lead to the 401 status code.
{% endhint %}
