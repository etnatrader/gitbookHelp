...
API requests in ETNA Trader require a unique authentication token to be provided in the header. Whether you want to create a price alert for a user, get a quote on a particular stock, or cancel an order — you first have to authorize the user with their unique token \(requests can only be made on behalf of some user\). This token can be generated by making a an API call where you provide a username and a password of a specific user. You will receive the above-mentioned token as a response which can be used in all other requests.

Every time you make a request, a fresh token will be generated and returned to you. Multiple tokens for one user can be used, as they will all be active until expiration.

{% hint style="info" %}
An authentication token lifetime is 24 hours. 
{% endhint %}
...
If the user account has two-factor authentication enabled, there're also two other mandatory parameters that must be provided in the header:
...
These two parameter don't have to be provided in the initial request because both authentication methods are performed in separate steps. In the first request, only username, password, and Et-App-Key must be provided. In response, you'll receive the following JSON file:
