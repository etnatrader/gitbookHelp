...
```text
curl -X POST --header 'Content-Type: application/x-www-form-urlencoded' --header 'Accept: application/json' --header 'Username: yourEmail' --header 'Password: yourPassword' --header 'Et-App-Key: yourAppKey' --header 'Content-Length: 0' 'https://pub-api-et-demo-prod.etnasoft.us/api/token'
```
